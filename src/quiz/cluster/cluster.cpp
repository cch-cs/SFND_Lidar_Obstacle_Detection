...
void proximity(const std::vector<std::vector<float>>& points, std::vector<int>& cluster, std::vector<bool>& processed, int i, float distanceTol, KdTree* tree){
	processed[i] = true;
	cluster.push_back(i);
	std::vector<int> neighbor_points;
	neighbor_points = tree->search(points[i],distanceTol);
...
int main ()
{
    ...
    for (int i=0; i<points.size(); i++) 
  	...
  	std::vector<int> nearby = tree->search({-6,7},5.0);
